...
(function (app) {
    ...
    var _ajax_load_content = function (url, callback) {
        ...
        var options = { "url": url, "callback": loadcallback, "block": true };
        /* 初始化页面元素 */
    ...
    app.form = {
        ...
        "initCache": function (options) {
            ...
            if (cfgs.cache[options.api] && options.cached) {
                ...
                App.ajax(options.api, { "nocache": true, "args": options.args, "block": false }, function (json) {
        ...
        "init": function () {
            ...
            // 刷新功能
            $(".btn_refresh").on("click", function () { App.table.reload(); });
        ...
        "openview": function (options) {
            ...
            $("#ajax-content").addClass("fadeOutRight");
            load_content({ "url": options.url, "callback": loadcallback, "block": true });
        }
        /** ajax方式加载页面内容并且处理菜单
         * @param {string} url  待加载内容的URL地址.
