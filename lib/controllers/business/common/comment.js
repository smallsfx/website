...
module.exports = {
  ...
  "add": (req, res) => {
    ...
    db.query(sql, params, function (err, result) {
      if (err) {
        dblogger.write(TEXT_COMMENT_LOST + err.message, req.hostname, req.session.user, TEXT_LOG_KEY);
        res.send(generator.message(err.message));
      } else {
        dblogger.write(TEXT_COMMENT_SUCCESS, req.hostname, req.session.user, TEXT_LOG_KEY);
  ...
  "delete": (req, res) => {
    ...
    db.query(sql, params, function (err, result) {
      if (err) {
        dblogger.write(TEXT_COMMENT_DELETE_LOST + err.message, req.hostname, req.session.user, TEXT_LOG_KEY);
        res.send(generator.message(err.message));
      } else {
        dblogger.write(TEXT_COMMENT_DELETE_SUCCESS, req.hostname, req.session.user, TEXT_LOG_KEY);
        res.send(generator.data());
      }
    });
  },
