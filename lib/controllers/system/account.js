...
module.exports = {
  ...
  "mapping": {
    "login": { "description": "用户登录", "auth": false },
    "logout": { "description": "用户注销登录", "auth": false },
    "changepwd": { "description": "用户修改密码", "auth": false },
    "changeinfo": { "description": "用户修改基本信息", "auth": false },
  ...
  "login": (req, res) => {
    ...
    if (!uid) {
      ...
      db.query(sql, params, function (err, rows, result) {
        if (err) {
          dblogger.write("登录失败," + err.message, req.hostname, uid, "LOGIN");
          ...
          if (rows.length == 1) {
            ...
            // 返回登录结果:用户基本信息
            dblogger.write("登录成功", req.hostname, uid, "LOGIN");
            ...
            dblogger.write("登录失败,密码不正确", req.hostname, uid, "LOGIN");
  ...
  "logout": (req, res) => {
    if (!req.session.user) {
      ...
      dblogger.write("注销成功", req.hostname, req.session.user, "LOGOUT");
  ...
  "changepwd": (req, res) => {
    ...
    if (!pwd) {
      ...
      db.query(sql, params, function (err, result) {
        if (err) {
          dblogger.write("密码修改失败," + err.message, req.hostname, uid, "CHANGEPWD");
          res.send(generator.message(err.message));
        } else {
          if (result.affectedRows == 1) {
            dblogger.write("密码修改成功", req.hostname, uid, "CHANGEPWD");
            res.send(generator.data());
          } else {
            dblogger.write("密码修改失败,密码不正确", req.hostname, uid, "CHANGEPWD");
  ...
  /** 用户修改基本信息
   * [@param](/user/param) req request
   * [@param](/user/param) res response
   */
  "changeinfo": (req, res) => {
    var uid = req.session.user;
    ...
    if (!username) {
      ...
      db.query(sql, params, function (err, result) {
        if (err) {
          dblogger.write("信息修改失败," + err.message, req.hostname, uid, "CHANGEPWD");
          res.send(generator.message(err.message));
        } else {
          if (result.affectedRows == 1) {
            dblogger.write("信息修改成功", req.hostname, uid, "CHANGEPWD");
            res.send(generator.data());
          } else {
            dblogger.write("信息修改失败,登录名不存在", req.hostname, uid, "CHANGEPWD");
