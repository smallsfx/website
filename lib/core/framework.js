...
var runModule = (m) => {
  ...
  if (define.dependenciesreg
    ...
    define.dependenciesreg.forEach((name, index) => {
  ...
  if (!define.dependencies || define.dependencies.length == 0) {
    ...
    define.dependencies.forEach((name, index) => {
      var dependence = undefined;

      if ('config' == name) {
        dependence = config;
      } else if (modulesMap[name].define.instand) {
        dependence = modulesMap[name].define.instand;
      }
  ...
  if (runResult) {
    logger.info("module " + m.define.name + " v" + m.define.version + " 导入成功");
    try {
      var execResult = m.run.apply(this, args);
    } catch (err) {
      logger.error("module " + m.define.name + " v" + m.define.version + " 启动失败",err);
    }
  ...
  while (isContinue) {
    ...
    Object.keys(modulesMap).map((key) => {
      ...
      if (!value.define.instand) {
        ...
        if (runResult) {
          modulesMap[key].define.instand = runResult;
        } else {
          isContinue = true;
        } // if (runResult) { } else {
      }// if (!value.define.instand) {
...
var importModules = (catalog) => {
  ...
  paths.forEach((_path) => {
    ...
    if (define && typeof _module.run === 'function') {
      ...
      logger.info("模块 " + _path + " 无效：未定义 define结构 或 run方法.");
    }
  });
...
var frameworkStart = () => {
  ...
  logger.info(config.name + " v" + config.version + " 启动");
  logger.info("根目录：" + config.rootPath);

  logger.info("初始化 模块 {modules}");
  ...
  logger.info("系统启动完成，耗时：" + (new Date().getTime() - begin) + " ms");
}
